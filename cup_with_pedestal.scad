// OpenSCAD Cup with Pedestal
// A decorative cup with a circular pedestal base
...
// Handle module
module handle() {
    translate([handle_offset, 0, cup_height * 0.4]) {
        rotate([0, 90, 0]) {
            difference() {
                // Outer torus for handle shape
                rotate_extrude($fn = 50) {
                    translate([15, 0, 0])
                        circle(r = handle_width/2, $fn = 20);
                }
                
                // Cut away part to create C-shape handle
                translate([-20, -25, -handle_width])
                    cube([40, 25, handle_width * 2]);
                
                // Cut away bottom part
                translate([-25, -25, -handle_width])
                    cube([50, 50, handle_width]);
            }
        }
    }
}
...
// Uncomment the line below to use decorative pedestal instead
// translate([0, 0, -pedestal_height]) decorative_pedestal();